...
class OfflinePlanChangeDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn('cooperative_name', function ($row) {

            return user()->is_superadmin ? '<a href="' . route('superadmin.offline-plan.show', $row->id) . '"  class="text-darkest-grey openRightModal">' . $row->cooperative->cooperative_name . '</a>' : $row->cooperative->cooperative_name;
        });
        ...
        $datatables->editColumn('created_at', function ($row) {
            return $row->created_at->setTimezone(cooperativeOrGlobalSetting()->timezone)->translatedFormat(cooperativeOrGlobalSetting()->date_format . ' ' . cooperativeOrGlobalSetting()->time_format);
        });

        $datatables->rawColumns(['cooperative_name', 'action', 'status']);
    ...
    {
        return $model->with(['cooperative' => function ($q) {
            $q->select('id', 'cooperative_name');
        }, 'package' => function ($q) {
            $q->select('id', 'name');
    ...
    protected function getColumns()
    {
        ...
        if (user()->is_superadmin) {
            $cooperative = [
                __('superadmin.cooperative') => ['data' => 'cooperative_name', 'name' => 'cooperative.cooperative_name', 'title' => __('superadmin.cooperative')],
            ];
        }
        ...
        return array_merge($data1, $cooperative, $data2);
