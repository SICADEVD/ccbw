...
class CooperativeDataTable extends BaseDataTable
{
    ...
    public function __construct()
    {
        ...
        $this->updatePackagesPermission = user()->permission('update_cooperative_package');
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn('action', function ($row) {
            ...
            if (module_enabled('Subdomain')) {
                $action .= '<a href="javascript:;" class="dropdown-item domain-params"
                    ...
                    data-cooperative-id="' . $row->id . '" data-cooperative-url="' . request()->getScheme() . '://' . $row->sub_domain . '" ><i class="fa fa-bell mr-2" aria-hidden="true"></i> ' . __('subdomain::app.core.sendDomainNotification') . '</a>';
            }
            ...
            if ($this->deleteCompaniesPermission == 'all') {
                ...
                $action .= '<a class="dropdown-item delete-table-row" href="javascript:;" data-cooperative-id="' . $row->id . '">
        ...
        $datatables->editColumn(
            'cooperative_name',
            function ($row) {
                return view('components.cooperative', [
                    'cooperative' => $row
                ]);
            }
        );
        ...
        $customFieldColumns = CustomField::customFieldData($datatables, Cooperative::CUSTOM_FIELD_MODEL);
        $datatables->rawColumns(array_merge(['cooperative_name', 'action', 'status', 'package', 'details'], $customFieldColumns));

        return $datatables;
    }

    /**
     * @param Cooperative $model
     * @return Cooperative|\Illuminate\Database\Eloquent\Builder|\Illuminate\Database\Query\Builder
     */
    public function query(Cooperative $model)
    {
        ...
        $companies = $model->newQuery()
            ...
            ->when($request->cooperativeStatus && $request->cooperativeStatus !== 'all', function ($query) use ($request) {
                return $query->where('status', $request->cooperativeStatus);
            })
            ...
            ->when($request->searchText, function ($query) use ($request) {
                return $query->where(function ($query) use ($request) {
                    ...
                    $query->where('cooperative_name', 'like', $searchText)
                        ->orWhere('cooperative_email', 'like', $searchText)
                        ->orWhere('cooperative_phone', 'like', $searchText);
                });
            });
    ...
    protected function getColumns()
    {
        $data = [
            ...
            __('app.cooperative') => ['data' => 'cooperative_name', 'name' => 'cooperative_name', 'title' => __('modules.accountSettings.cooperativeName')],
        ...
        return array_merge($data, CustomFieldGroup::customFieldsDataMerge(new Cooperative()), $action);
    }
