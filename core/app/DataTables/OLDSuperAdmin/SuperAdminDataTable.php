...
class SuperAdminDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $roles = Role::withoutGlobalScopes([CooperativeScope::class])->whereNull('cooperative_id')->get();
    ...
    public function query(User $model)
    {
        ...
        return $model->newQuery()
            ...
            ->withoutGlobalScopes([ActiveScope::class, CooperativeScope::class])
            ->where(function ($query) use ($searchText) {
                $query->where('users.name', 'like', "%$searchText%")
                    ->orWhere('email', 'like', "%$searchText%");
            })
            ->where('is_superadmin', 1)
            ->whereNull('users.cooperative_id');
    }
