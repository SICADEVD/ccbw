...
class InvoiceDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ->eloquent($query)
            ->addIndexColumn()
            ->addColumn('cooperative', function ($row) {
                return ucfirst($row->cooperative->cooperative_name);
            })
    ...
    public function query(GlobalInvoice $model)
    {
        ...
        $globalInvoice = $model->with('package', 'cooperative', 'currency', 'offlinePaymentMethod')
            ->whereNotNull('pay_date');


        if ($cooperativeId && $cooperativeId != 'all') {
            $globalInvoice->where('cooperative_id', $cooperativeId);
        }
    ...
    protected function getColumns()
    {
        ...
        if (user()->is_superadmin) {
            $cooperative = [
                __('superadmin.cooperative') => ['data' => 'cooperative', 'name' => 'cooperative', 'title' => __('superadmin.cooperative')],
            ];
        }
