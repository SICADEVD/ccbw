...
class EmployeeShiftScheduleController extends AccountBaseController
{
    ...
    public function summaryData($request)
    {
        ...
        $now = now()->timezone($this->cooperative->timezone);
    ...
    public function weekSummaryData($request)
    {
        ...
        $now = Carbon::parse($request->week_start_date, cooperative()->timezone);
        ...
        foreach ($employees as $employee) {
            ...
            $joiningDate = Carbon::createFromFormat('Y-m-d', $employee->employeeDetail->joining_date->toDateString(), cooperative()->timezone)->startOfDay();
    ...
    {
        if($viewType == 'week'){
            $now = Carbon::parse($startDate, cooperative()->timezone);
    ...
    public function create()
    {
        ...
        $this->emailSetting = EmailNotificationSetting::where('cooperative_id', $this->cooperative->id)->where('slug', 'shift-assign-notification')->first();
    ...
    public function bulkShift(StoreBulkShift $request)
    {
        ...
        else if($request->assign_shift_by == 'date')
        {
            ...
            $period[] = $singleDate = Carbon::createFromFormat(cooperative()->date_format, $request->single_date);
            ...
            $sDate = Carbon::createFromFormat(cooperative()->date_format, $request->startDate);
            $eDate = Carbon::createFromFormat(cooperative()->date_format, $request->endDate);
        ...
        if ($request->send_email && $insertData > 0) {
            foreach ($employees as $key => $userId) {
                ...
                if (smtp_setting()->mail_connection == 'sync') {
                    Mail::to($userData->email)->send(new BulkShiftEmail($dateRange, $userId, cooperative()));

                } else {
                    Mail::to($userData->email)->queue(new BulkShiftEmail($dateRange, $userId, cooperative()));
                }
            }
        }
