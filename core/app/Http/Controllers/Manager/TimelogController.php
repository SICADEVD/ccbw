...
class TimelogController extends AccountBaseController
{
    ...
    {
        $startDateTime = Carbon::createFromFormat($this->cooperative->date_format, $request->start_date, $this->cooperative->timezone)->format('Y-m-d') . ' ' . Carbon::createFromFormat($this->cooperative->time_format, $request->start_time)->format('H:i:s');
        $startDateTime = Carbon::parse($startDateTime, $this->cooperative->timezone)->setTimezone('UTC');

        $endDateTime = Carbon::createFromFormat($this->cooperative->date_format, $request->end_date, $this->cooperative->timezone)->format('Y-m-d') . ' ' . Carbon::createFromFormat($this->cooperative->time_format, $request->end_time)->format('H:i:s');
        $endDateTime = Carbon::parse($endDateTime, $this->cooperative->timezone)->setTimezone('UTC');
    ...
    public function startTimer(StartTimer $request)
    {
        ...
        if (is_null($activeTimer) || (!is_null($activeTimer) && !is_null($activeTimer->activeBreak))) {
            ...
            if ($request->has('create_task')) {
                ...
                $task->board_column_id = $this->cooperative->default_task_status;
                $task->is_private = $request->has('is_private') && $request->is_private == 'true' ? 1 : 0;
                $task->start_date = Carbon::now($this->cooperative->timezone)->format('Y-m-d');
                $task->due_date = Carbon::now($this->cooperative->timezone)->format('Y-m-d');
    ...
    public function byEmployee()
    {
        ...
        $this->startDate = now()->startOfMonth()->format(cooperative()->date_format);

        $this->endDate = now()->format(cooperative()->date_format);
    ...
    public function employeeData(Request $request)
    {
        if(is_null($request->startDate) && is_null($request->endDate)){
            $timeSheetStartDate = now()->startOfMonth()->format(cooperative()->date_format);
            $timeSheetEndDate = now()->format(cooperative()->date_format);
        }
        else {
        ...
        $startDate = Carbon::createFromFormat($this->cooperative->date_format, $timeSheetStartDate)->toDateString();
        $endDate = Carbon::createFromFormat($this->cooperative->date_format, $timeSheetEndDate)->toDateString();
    ...
    {
        $startDate = Carbon::createFromFormat($this->cooperative->date_format, $request->startDate)->startOfDay()->toDateTimeString();
        $endDate = Carbon::createFromFormat($this->cooperative->date_format, $request->endDate)->endOfDay()->toDateTimeString();
        $employee = $request->employee;
        $projectId = $request->projectID;
