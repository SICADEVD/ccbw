...
class TimeLogSettingController extends AccountBaseController
{
    ...
    public function index()
    {
        ...
        $this->time = $this->data['logTime']->time ? Carbon::createFromFormat('H:i:s', $this->data['logTime']->time)->format(cooperative()->time_format) : '';
    ...
    public function store(UpdateTimeLog $request)
    {
        ...
        if ($request->has('tracker_reminder')) {
            ...
            $logTime->time = Carbon::createFromFormat($this->cooperative->time_format, $request->time)->format('H:i:s');
        }
