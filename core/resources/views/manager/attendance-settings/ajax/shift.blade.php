            ...
            <tr class="row{{ $shift->id }}">
                ...
                <td>
                    <div class="f-11">@lang('modules.employees.startTime'):
                        {{ \Carbon\Carbon::createFromFormat('H:i:s', $shift->office_start_time)->translatedFormat('H:i') }}
                    </div>
                    <div class="f-11">
                        @lang('modules.attendance.halfDay'):
                        {{ $shift->halfday_mark_time? \Carbon\Carbon::createFromFormat('H:i:s', $shift->halfday_mark_time)->translatedFormat('H:i'): '' }}
                    </div>
                    <div class="f-11">
                        @lang('modules.employees.endTime'):
                        {{ \Carbon\Carbon::createFromFormat('H:i:s', $shift->office_end_time)->translatedFormat('H:i') }}
                    </div>
                </td>
                <td>
                    <div class="f-11">
...
<script>
    ...
    $('body').on('click', '.delete-shift', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                var url = "{{ route('manager.employee-shifts.destroy', ':id') }}";
    ...
    $('body').on('click', '.set_default_shift', function() {
        ...
        $.easyAjax({
            url: "{{ route('manager.employee-shifts.set_default') }}",
            type: "POST",
            data: {
                shiftID: shiftID,
                _token: token
